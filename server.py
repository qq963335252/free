...
class ChatRoom(Room):
    ...
    def add(self, session):
        ...
        self.broadcast((session.name + '进入了当前聊天室\n').encode("utf-8"))
        self.server.users[session.name] = session
        Room.add(self, session)
       #speaker = win32com.client.Dispatch("SAPI.SpVoice")
       #speaker.Speak(session.name + '进入了当前聊天室')

    def remove(self, session):
        # 广播用户离开
        Room.remove(self, session)
        self.broadcast((session.name + '离开了当前聊天室.\n').encode("utf-8"))
     #  speaker = win32com.client.Dispatch("SAPI.SpVoice")
      # speaker.Speak(session.name + '离开了当前聊天室')

    def do_say(self, session, line):
        # 客户端发送消息
        self.broadcast((session.name + ' :     ' + time.strftime("%Y-%m-%d %H:%M:%S")+'\n'+'    ' + line + '\n'+ '\n').encode("utf-8"))
    ...
    def do_tuling_say(self, session, line):
        # 图灵回复消息
        self.broadcast(('华软小E :     '+time.strftime("%Y-%m-%d %H:%M:%S")+'\n'+'    ' + line + '\n'+ '\n').encode("utf-8"))
